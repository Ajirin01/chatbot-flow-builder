  ...
  methods: {
    isSelected(message) {
      if(this.selectedObject){
        return this.selectedObject.next && this.selectedObject.next.id === message.id;
      }
    },
    toggleSelection(message) {
      if (this.isSelected(message)) {
        this.selectedObject.next = null; // Deselect the message
      } else {
        this.selectedObject.next = message; // Select the message
      }
    },
    assignNextMessage() {
      console.log("Assigned next message:", this.selectedObject.next, "to option:", this.selectedOption);
    },
  },
  computed: {
